...
func (p *Provider) Start(ctx context.Context, aware multicluster.Aware) error {
	if err := p.run(ctx, aware); err != nil {
...
func (p *Provider) RunOnce(ctx context.Context, aware multicluster.Aware) error {
	return p.run(ctx, aware)
}

func (p *Provider) addCluster(ctx context.Context, aware multicluster.Aware, name string, cl cluster.Cluster) {
	...
	for name, cl := range loadedClusters {
		if slices.Contains(knownClusters, name) {
			...
			if !cmp.Equal(existingCluster.GetConfig(), cl.GetConfig()) {
				...
				p.removeCluster(name)
				p.addCluster(ctx, aware, name, cl)
			}
			continue
		}
